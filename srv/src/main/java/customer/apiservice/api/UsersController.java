...
public class UsersController {
  ...
  /** GET /users - GET all users with parameters */
  ...
  public ResponseEntity<Map<String, Object>> getUsers(
    ...
    List<Map<String, Object>> rows =
        users.findUsers(startIndex, count, search, email, status, userType, country);
  ...
  public ResponseEntity<Map<String, Object>> getUserById(@PathVariable("id") String id) {
    return users
        ...
        .orElseGet(
            () -> ResponseEntity.status(404).body(Map.of("error", "User not found", "id", id)));
  }
  ...
  public ResponseEntity<Map<String, Object>> createUser(@RequestBody Map<String, Object> userData) {
    ...
    try {
      ...
      ResponseEntity<Map<String, Object>> requiredError =
          FieldValidator.checkRequired(userData, "lastName", "email", "userType", "loginName");
      if (requiredError != null) {
        return requiredError;
      }

      // Validate email format
      ResponseEntity<Map<String, Object>> emailError =
          FieldValidator.checkEmail((String) userData.get("email"));
      if (emailError != null) {
        return emailError;
      }

      if (!userData.containsKey("status")
          || userData.get("status") == null
      ...
      if (iasResponse.statusCode() != 201) {
        log.error(
            "IAS user creation failed: {} - {}", iasResponse.statusCode(), iasResponse.body());
  ...
  private Map<String, Object> extractUserDataFromScim(JsonNode scimUser) {
    ...
    // IAS metadata
    user.put(
        "iasLastModified", parseTimestamp(scimUser.path("meta").path("lastModified").asText(null)));
  ...
  public ResponseEntity<Map<String, Object>> updateUser(
      @PathVariable("id") String id, @RequestBody Map<String, Object> updates) {
    ...
    try {
      ...
      ResponseEntity<Map<String, Object>> emailError =
          FieldValidator.checkEmail((String) updates.get("email"));
      if (emailError != null) {
        return emailError;
      }

      // Validate status if provided
      ResponseEntity<Map<String, Object>> statusError =
          FieldValidator.checkStatus((String) updates.get("status"));
      if (statusError != null) {
        return statusError;
      }
      ...
      return ResponseEntity.status(HttpStatus.INTERNAL_SERVER_ERROR)
          .body(Map.of("error", "Failed to update user", "message", e.getMessage(), "id", id));
    }
  }
    ...
    try {
      ...
      if (rowsDeleted == 0) {
        return ResponseEntity.status(HttpStatus.INTERNAL_SERVER_ERROR)
            .body(
                Map.of(
                    "error", "User deleted from IAS but failed to delete from database", "id", id));
      }
      ...
      return ResponseEntity.ok(
          Map.of(
              ...
              loginName,
              "deletedFromIAS",
              iasResponse.statusCode() == 204 || iasResponse.statusCode() == 200,
              "deletedFromDatabase",
              true));

    } catch (Exception e) {
      return ResponseEntity.status(HttpStatus.INTERNAL_SERVER_ERROR)
          .body(Map.of("error", "Failed to delete user", "message", e.getMessage(), "id", id));
    }
  }
  ...
  public ResponseEntity<Map<String, Object>> addUserToGroups(
    ...
    try {
      ...
      ResponseEntity<Map<String, Object>> groupIdsError =
          FieldValidator.checkRequired(request, "groupIds");
      ...
      for (String groupId : groupIds) {
        ...
        // We are adding one user to one group
        ...
        if (iasResponse.statusCode() < 200 || iasResponse.statusCode() >= 300) {
          log.error(
              "Failed to add user to group in IAS: {} - {}",
  ...
  public ResponseEntity<Map<String, Object>> removeUserFromGroups(
    ...
    try {
      ...
      //  Remove user from each group in IAS
      ...
      for (String groupId : groupIds) {
        ...
        if (iasResponse.statusCode() < 200 || iasResponse.statusCode() >= 300) {
          log.error(
              "Failed to remove user from group in IAS: {} - {}",
              iasResponse.statusCode(),
              iasResponse.body());
          failed.add(groupId);
        } else {
